...
abstract class Decoder<TData> implements DecoderInt<TData> {
  ...
  public abstract decode(dataArray: Uint8Array, initialPosition?: number, maxLength?: number): ChartData<TData>;
  ...
  protected uint(buf: Uint8Array) {
    ...
    if (l = (l | (127 & buf[this.position]) << 7) >>> 0,
    buf[this.position++] < 128) {
      return l;
    }
    if (l = (l | (127 & buf[this.position]) << 14) >>> 0,
    buf[this.position++] < 128) {
      return l;
    }
    if (l = (l | (127 & buf[this.position]) << 21) >>> 0,
    buf[this.position++] < 128) {
      return l;
    }
    if (l = (l | (15 & buf[this.position]) << 28) >>> 0,
    buf[this.position++] < 128) {
      return l;
    }
    return l;
...
export class LineDecoder extends Decoder<number> {
  decode(dataArray: Uint8Array, initialPosition?: number, maxLength?: number): ChartData<number> {
    ...
    while (this.position < length) {
      ...
      switch (t >>> 3) {
        case 1:
          console.log(this.getString(dataArray));
          break;
        ...
        case 3:
          ...
          break;
        case 4:
          console.log(this.getInt(dataArray));
          break;
        default:
          console.log(t & 7);
      }
  ...
  decode(dataArray: Uint8Array, initialPosition?: number, maxLength?: number): ChartData<number> {
    ...
    while (this.position < length) {
      ...
      switch (index >>> 3) {
        ...
        case 2:
          chartData = lineDecoder.decode(dataArray, this.position, this.position + this.uint(dataArray));
